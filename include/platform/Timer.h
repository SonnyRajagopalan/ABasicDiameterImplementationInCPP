...
class Timer
{
 public:
  ...
  // Removes the timer from the TimerTask
  void cancel ();
  ...
  inline static UINT64 getIndex (Timer* _pTimer) { return pTimerPool->getIndexFromMemoryLocation (static_cast<void*> (_pTimer)); } // Index of the Timer object in the mem pool
  inline static Timer* getTimerAtIndex (UINT64 _index) { return static_cast<Timer*>(pTimerPool->getMemoryLocationFromIndex (_index)); }
